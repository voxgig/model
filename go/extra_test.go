...
func TestReloadReResolves(t *testing.T) {
	...
	b := NewBuild(BuildSpec{
		Path: filepath.Join(dir, "m.aontu"), Base: dir, Resolver: r,
...
func TestResolverError(t *testing.T) {
	...
	b := NewBuild(BuildSpec{Path: filepath.Join(dir, "m.aontu"), Base: dir, Resolver: r})
	br := b.Run(false)
	if br.OK {
...
func TestMissingRootFile(t *testing.T) {
	dir := t.TempDir()
	b := NewBuild(BuildSpec{Path: filepath.Join(dir, "nope.aontu"), Base: dir})
	br := b.Run(false)
	if br.OK {
...
func TestActionSteps(t *testing.T) {
	...
	b := NewBuild(BuildSpec{
		Path: filepath.Join(dir, "m.aontu"), Base: dir,
		Actions: map[string]ActionDef{
...
func TestModelProducerWriteError(t *testing.T) {
	...
	writeFile(t, dir, "m.aontu", "a: 1\n")
	b := NewBuild(BuildSpec{
		Path: filepath.Join(dir, "m.aontu"), Base: dir, FS: failWriteFS{},
...
func TestModelDryrunAndBuild(t *testing.T) {
	dir := t.TempDir()
	writeFile(t, dir, "m.aontu", "a: 1\n")
	m := New(ModelSpec{Path: filepath.Join(dir, "m.aontu"), Base: dir, Dryrun: true})
