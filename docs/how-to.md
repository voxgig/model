...
Scaffold a starter `model/model.aon` and `model/.model-config/model-config.aon`:
...
```bash
voxgig-model --watch model/model.aon
```
...
```js
new Model({ path: 'model/model.aon', base: 'model',
...
1. Declare it in `model/.model-config/model-config.aon`:
...
Sometimes an action must generate `.aon` that the model itself consumes. Run
...
module.exports = async function genSource(model, build) {
  const root = Path.resolve(build.path, '..', '..')
  // write a .aon file the root model imports
  build.fs.writeFileSync(
    Path.resolve(root, 'model', 'generated.aon'),
    'generated: ok: true\n'
  )
...
```js
new Model({ path: 'model/model.aon', base: 'model', dryrun: true })
```
...
const { fs, vol } = memfs({
  '/proj/model/model.aon': 'service: name: orders\n',
  '/proj/model/.model-config/model-config.aon': 'sys: model: action: {}\n',
})

const model = new Model({
  path: '/proj/model/model.aon',
  base: '/proj/model',
  fs,
})
await model.run()
...
```bash
voxgig-model -g debug model/model.aon     # verbose
voxgig-model -g warn  model/model.aon     # quieter
voxgig-model -g silent model/model.aon    # no logs
```
