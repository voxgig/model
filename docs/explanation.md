...
| `Config` | `ts/src/config.ts` | A specialized build for `.model-config/model-config.aon`. |
...
*generate model source*. A code generator might emit a `.aon` fragment that
...
that list is itself modeled, in `.model-config/model-config.aon`. So a `Model`
resolves **two** models:
